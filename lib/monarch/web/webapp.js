...
var pup_tent = require('pup-tent')(['js','css','templates','templates/labs', 'templates/legacy',
                                    ...
                                    'node_modules/phenogrid/dist',
                                    'node_modules/phenogrid/config',
                                    'node_modules/phenogrid/css',
                                    'node_modules/phenogrid/fonts',
                                    'node_modules/phenogrid/image',
...
function addPhenogridFiles(info) {
    ...
    info.pup_tent_js_libraries.push("/node_modules/phenogrid/config/phenogrid_config.js");
    info.pup_tent_js_libraries.push("/node_modules/phenogrid/dist/phenogrid-byo.js");
    info.pup_tent_css_libraries.push("/node_modules/phenogrid/css/phenogrid.css");
    info.pup_tent_css_libraries.push("/node_modules/phenogrid/css/normalize.css");
    info.pup_tent_css_libraries.push("/node_modules/phenogrid/css/instructions.css");
    info.pup_tent_css_libraries.push("/node_modules/phenogrid/css/font-awesome.css");
    info.pup_tent_css_libraries.push("/node_modules/phenogrid/css/stickytooltip.css");
	...
	// info.pup_tent_css_libraries.push("/font-awesome.css");
}
...
// anything in the widgets/ directory is passed through statically
// Note: This code actually applies Mustache templating to this content, which is probably
// a mistake, since some of these assets may be PNG or binary assets that contain directives
// (due to sheer randomness) that Mustache may alter. This function should be more careful.
//
app.get('/widgets/*', function(request) {
    engine.log('### widgets' + request);
    var path = request.pathInfo;
    var ct = _decide_content_type(path);
    return serveDirect(".", path, ct);
});
...
app.get('/disease', function(request) {
    ...
    var disPhenoDist = JSON.parse(fs.read('./widgets/dove/stats/key-disease-phenotype-distro.json'));

    info.pup_tent_js_variables.push({name:'globalDataGraph',value:disPhenoDist});
    info.monarch_launchable = [];
...
var fetchModelPage = function(request, id, fmt) {
    try {
        ...
        addGolrTable(info, "object_closure", id, 'disease-table', disease_filter, 'disease_model');
        ...
        addGolrTable(info, "subject_closure", id, 'phenotypes-table', phenotype_filter, 'model_phenotype');
        ...
        addGolrTable(info, "object_closure", id, 'gene-table', gene_filter, 'gene_model');
        ...
        addGolrTable(info, "object_closure", id, 'variant-table', variant_filter, 'variant_model');
        ...
        addGolrTable(info, "subject_closure", id, 'pathway-table', pathway_filter, 'model_pathway');
...
app.get('/gene', function(request) {
    ...
    var diseaseDist = JSON.parse(fs.read('./widgets/dove/stats/key-disease-gene-distro.json'));
...
var fetchModelLandingPage = function (request){
    ...
    info.pup_tent_js_libraries.push("/barchart-launcher.js");
    info.pup_tent_js_libraries.push("/graph-config.js");
    var phenoGenoDist = JSON.parse(fs.read('./widgets/dove/stats/key-pheno-genotype-distro.json'));

    info.pup_tent_js_variables.push({name:'globalDataGraph',value:phenoGenoDist});
    info.monarch_launchable = [];
...
function addGolrStaticFiles(info) {
    var golr_conf_raw = pup_tent.get('conf/golr-conf-beta.json');
...
function addGolrTable(info, query_field, id, div, filter, personality) {
...
app.get('/phenotype/:id.:fmt?',
        function(request, id, fmt){
            ...
            //Load variables for client side tables
            var disease_filter = { field: 'subject_category', value: 'disease' };
            addGolrTable(info, "object_closure", id, 'disease-table', disease_filter, 'disease_phenotype');
            info.diseaseNum = engine.fetchAssociationCount(id, 'object_closure', disease_filter);
            
            var gene_filter = { field: 'subject_category', value: 'gene' };
            addGolrTable(info, "object_closure", id, 'gene-table', gene_filter, 'gene_phenotype');
            info.geneNum = engine.fetchAssociationCount(id, 'object_closure', gene_filter);
            
            var genotype_filter = { field: 'object_category', value: 'genotype' };
            addGolrTable(info, "subject_closure", id, 'genotype-table', genotype_filter, 'phenotype_genotype');
            ...
            addGolrTable(info, "subject_closure", id, 'model-table', model_filter, 'phenotype_model');
            info.modelNum = engine.fetchAssociationCount(id, 'subject_closure', model_filter);
            
            var variant_filter = { field: 'subject_category', value: 'variant' };
            addGolrTable(info, "object_closure", id, 'variant-table', variant_filter, 'variant_phenotype');
            info.variantNum = engine.fetchAssociationCount(id, 'object_closure', variant_filter);
            
            var pathway_filter = { field: 'object_category', value: 'pathway' };
            addGolrTable(info, "subject_closure", id, 'pathway-table', pathway_filter, 'phenotype_pathway');
            info.pathwayNum = engine.fetchAssociationCount(id, 'subject_closure', pathway_filter);
...
app.get('/disease/:id.:fmt?',
        ...
        try {
            ...
            //Load variables for client side tables
            var phenotype_filter = { field: 'object_category', value: 'phenotype' };
            addGolrTable(info, "subject_closure", id, 'phenotypes-table', phenotype_filter, 'disease_phenotype');
            info.phenotypeNum = engine.fetchAssociationCount(id, 'subject_closure', phenotype_filter);
            
            var gene_filter = { field: 'subject_category', value: 'gene' };
            addGolrTable(info, "object_closure", id, 'gene-table', gene_filter, 'gene_disease');
            info.geneNum = engine.fetchAssociationCount(id, 'object_closure', gene_filter);
            
            var genotype_filter = { field: 'object_category', value: 'genotype' };
            addGolrTable(info, "subject_closure", id, 'genotype-table', genotype_filter, 'disease_genotype');
            info.genotypeNum = engine.fetchAssociationCount(id, 'subject_closure', genotype_filter);
            
            var model_filter = { field: 'object_category', value: 'model' };
            addGolrTable(info, "subject_closure", id, 'model-table', model_filter, 'disease_model');
            info.modelNum = engine.fetchAssociationCount(id, 'subject_closure', model_filter);
            
            var variant_filter = { field: 'subject_category', value: 'variant' };
            addGolrTable(info, "object_closure", id, 'variant-table', variant_filter, 'variant_disease');
            info.variantNum = engine.fetchAssociationCount(id, 'object_closure', variant_filter);
            
            var pathway_filter = { field: 'object_category', value: 'pathway' };
            addGolrTable(info, "subject_closure", id, 'pathway-table', pathway_filter, 'disease_pathway');
            info.pathwayNum = engine.fetchAssociationCount(id, 'subject_closure', pathway_filter);
...
var fetchFeaturePage = function(request, id, fmt) {
    try {
        ...
        //Load variables for client side tables
        var phenotype_filter = { field: 'object_category', value: 'phenotype' };
        addGolrTable(info, "subject_closure", id, 'phenotypes-table', phenotype_filter, 'gene_phenotype');
        info.phenotypeNum = engine.fetchAssociationCount(id, 'subject_closure', phenotype_filter);
        
        var disease_filter = { field: 'object_category', value: 'disease' };
        addGolrTable(info, "subject_closure", id, 'disease-table', disease_filter, 'gene_disease');
        info.diseaseNum = engine.fetchAssociationCount(id, 'subject_closure', disease_filter);
        
        var genotype_filter = { field: 'object_category', value: 'genotype' };
        addGolrTable(info, "subject_closure", id, 'genotype-table', genotype_filter, 'gene_genotype');
        info.genotypeNum = engine.fetchAssociationCount(id, 'subject_closure', genotype_filter);
        
        var model_filter = { field: 'object_category', value: 'model' };
        addGolrTable(info, "subject_closure", id, 'model-table', model_filter, 'gene_model');
        info.modelNum = engine.fetchAssociationCount(id, 'subject_closure', model_filter);
        
        var variant_filter = { field: 'subject_category', value: 'variant' };
        addGolrTable(info, "object_closure", id, 'variant-table', variant_filter, 'variant_gene');
        info.variantNum = engine.fetchAssociationCount(id, 'object_closure', variant_filter);
        
        var homolog_filter = { field: 'object_category', value: 'gene' };
        addGolrTable(info, "subject_closure", id, 'homolog-table', homolog_filter, 'gene_homolog');
        info.homologNum = engine.fetchAssociationCount(id, 'subject_closure', homolog_filter);
        
        var pathway_filter = { field: 'object_category', value: 'pathway' };
        addGolrTable(info, "subject_closure", id, 'pathway-table', pathway_filter, 'gene_pathway');
        info.pathwayNum = engine.fetchAssociationCount(id, 'subject_closure', pathway_filter);
...
var fetchVariantPage = function(request, id, fmt) {
    try {
        ...
        //Load variables for client side tables
        var phenotype_filter = { field: 'object_category', value: 'phenotype' };
        addGolrTable(info, "subject_closure", id, 'phenotypes-table', phenotype_filter, 'variant_phenotype');
        info.phenotypeNum = engine.fetchAssociationCount(id, 'subject_closure', phenotype_filter);
        
        var disease_filter = { field: 'object_category', value: 'disease' };
        addGolrTable(info, "subject_closure", id, 'disease-table', disease_filter, 'variant_disease');
        info.diseaseNum = engine.fetchAssociationCount(id, 'subject_closure', disease_filter);
        
        var genotype_filter = { field: 'object_category', value: 'genotype' };
        addGolrTable(info, "subject_closure", id, 'genotype-table', genotype_filter, 'variant_genotype');
        info.genotypeNum = engine.fetchAssociationCount(id, 'subject_closure', genotype_filter);

        var model_filter = { field: 'object_category', value: 'model' };
        addGolrTable(info, "subject_closure", id, 'model-table', model_filter, 'variant_model');
        info.modelNum = engine.fetchAssociationCount(id, 'subject_closure', model_filter);
        
        var pathway_filter = { field: 'object_category', value: 'pathway' };
        addGolrTable(info, "subject_closure", id, 'pathway-table', pathway_filter, 'variant_pathway');
        info.pathwayNum = engine.fetchAssociationCount(id, 'subject_closure', pathway_filter);
